...
contains
    ...
    subroutine render_welcome_menu(favorites, fav_count, recents, rec_count, &
        ...
        if (item_count == 0) then
            ...
            do i = 1, min(visible_height, item_count)
                if (i + scroll_offset > item_count) exit

                call terminal_move_cursor(display_row, 1)
                if (i + scroll_offset == selected_index) then
                    ! Highlight selected
                ...
                if (showing_favorites) then
                    write(line, '(A,A,I2,A,A,A,A)') trim(line), '  [', i + scroll_offset, '] ', &
                        trim(favorites(i + scroll_offset)%label), ' → ', &
                        trim(favorites(i + scroll_offset)%path)
                else
                    write(line, '(A,A,I2,A,A,A,A)') trim(line), '  [', i + scroll_offset, '] ', &
                        trim(recents(i + scroll_offset)%label), ' → ', &
                        trim(recents(i + scroll_offset)%path)
                end if
    ...
    subroutine adjust_scroll(selected, offset, visible_height)
        ...
        if (selected > offset + visible_height - margin) then
            offset = selected - visible_height + margin
        end if

        ! Scroll up if needed
        if (selected < offset + margin + 1) then
            offset = max(0, selected - margin - 1)
        end if
